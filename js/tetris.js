var tetris={
  ...
  paintState:function(){
    if(this.state!=this.RUNNING){
      var img=new Image();
      img.src=this.state==this.GAMEOVER?"img/game-over.png":
                                 "img/pause.png";
      //debugger;
  ...
  paintScore:function(){
    document.querySelector(".playground>p:first-child>span")
      .innerHTML=this.score;
    document.querySelector(".playground>p:nth-child(2)>span")
      .innerHTML=this.ln;
  },
  deleteRows:function(){
    for(var r=this.RN-1,ln=0;r>=0&&this.wall[r].join("")!=""&&ln<4;
        r--){
      if(this.isFullRow(r)){
        this.deleteRow(r);
        ln++;
